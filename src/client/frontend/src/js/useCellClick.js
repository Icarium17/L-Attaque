...
export function useCellClick({ board, setBoard, turn, setTurn, selectedCell, setSelectedCell, loading, setLoading, phase, setError, handlePlacementCellClick, isLake, playerColor, playerOrder, opponentColor, setPingMs }) {

  const dragSource = useRef(null);

  const applyMoveResult = (data, t) => {
    ...
    if (status != "BATTLE" && data?.result?.apiBoard)
  ...
  const handleCellClick = (row, col) => {
    ...
    if (!selectedCell) {
      ...
      makeMove(selectedCell.row, selectedCell.col, row, col)
        .then((data) => applyMoveResult(data, t))
